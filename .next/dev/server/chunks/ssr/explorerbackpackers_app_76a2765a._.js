...
// --- NEW: Define types for the booking objects ---
__turbopack_context__.s([
...
const checkRoomAvailability = async (startDate, endDate, adults, promoCode)=>{
    ...
    try {
        ...
        // The Firebase function already returns the propertyRooms array
        const data = await response.json();
        return data;
    } catch (error) {
        console.error("Error fetching room availability from Firebase:", error);
        throw error;
    }
};
async function processYocoPayment(bookingDetails) {
    const { totalCost, firstName, lastName, email, phone, country, estimatedArrivalTime, note, roomTypeId, numberOfGuests, checkIn, checkOut } = bookingDetails;
    // Use URLSearchParams to handle encoding of special characters in notes
    const params = new URLSearchParams({
    ...
    const url = `https://us-central1-explorer-backpackers.cloudfunctions.net/processYocoPayment?${params.toString()}`;
    try {
...
async function processYocoTourPayment(tourBookingDetails) {
    const { totalCost, firstName, lastName, email, phone, tourName, tourDate, guests, note } = tourBookingDetails;
    ...
    const url = `https://us-central1-explorer-backpackers.cloudfunctions.net/processYocoTourPayment?${params.toString()}`;
    try {
        const response = await fetch(url, {
            method: 'POST',
            headers: {
        ...
        console.error('Error creating tour checkout: ', error);
        throw error;
    }
}
}),
...
//# sourceMappingURL=explorerbackpackers_app_76a2765a._.js.map