...
import { spasData } from '../data/spaData';
  ...
  return (
    <main>
      <HeroSection
        title="Spas & Wellness"
        subtitle="Relax"
        emphasis="Pamper Yourself"
        buttonText="View Spas"
        onButtonClick={() => document.getElementById('spas-list')?.scrollIntoView({ behavior: 'smooth' })}
        // Fallback check for the util function
        bgImage={getHeroImageForCategory ? getHeroImageForCategory('Spa') : ''} 
      />
      
      <section id="spas-list" className="py-16 lg:py-24 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {displayedSpas.length > 0 ? (
              displayedSpas.map((item: any, index: number) => (
                <InfoCard key={item.title || index} item={item} />
              ))
            ) : (
              <p className="text-center text-lg text-gray-600 mt-8">No venues found for this category.</p>
            )}
          </div>
        </div>
      </section>
...
export default function SpaPage() {
  return (
    <>
      <PageTitle title="Spas & Wellness" />
      <Suspense fallback={<div className="p-20 text-center">Loading wellness centers...</div>}>
        <SpaContent />
      </Suspense>
    </>
  );
