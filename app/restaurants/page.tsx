...
function RestaurantsContent() {
  ...
  const safeData = restaurantsData || [];
  ...
  return (
    <main>
      <HeroSection
        ...
        emphasis="Where to Eat"
        buttonText="View Restaurants"
        onButtonClick={() => document.getElementById('restaurants-list')?.scrollIntoView({ behavior: 'smooth' })}
        // Fallback check for the util function
        bgImage={getHeroImageForCategory ? getHeroImageForCategory('Restaurants') : ''} 
      />
      
      <section id="restaurants-list" className="py-16 lg:py-24 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {displayedRestaurants.length > 0 ? (
              ...
              <p className="text-center text-lg text-gray-600 mt-8">No restaurants found for this category.</p>
            )}
          </div>
        </div>
      </section>
...
export default function RestaurantsPage() {
  return (
    <>
      ...
      <Suspense fallback={<div className="p-20 text-center">Loading restaurants...</div>}>
        <RestaurantsContent />
      </Suspense>
    </>
  );
